...
### 3. 초음파센서 실습
- 초음파센서 원리
	- 센서의 송신부(Trig)에서 일정한 시간의 간격을 둔 짧은 초음파 펄스를 방사하고, 대상물에 부딪혀 돌아온 에코 신호를 수신부(Echo)에서 받아, 이에 대한 시간차를 기반으로 거리 산출
...
### 1. 릴레이모듈 실습
- 릴레이모듈 원리
NC(normal close), 닫힌접점 - 를 쓰고싶으면 COM+NC끼리 연결, vcc랑 연결되면 켜짐
NO(normal open), 열린접점 - COM+NO, vcc 연결해도 안켜짐(접점 연결x), 접점 연결시키는 놈들이 핀3개(코일역할)-> 전류를 흘리면 자기장이 생성되니깐(전자석이 되~) 이 원리를 이용해서 열려있는 철판? 암튼 끌어당겨서 접점 연결시키게 됨... 반대쪽은 열리게 되겟지
이기종을 제어할떄,,,(12v, 24v를 제어)
(회로연결 어렵넹...;;)
